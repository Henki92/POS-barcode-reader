    ...
    def search_article(self, Event):
        ...
        if len(self.search_entry.get()) > 0:
            if 'paket' in self.search_entry.get():
                ...
                #print(spin_boxes)
                #print('set spinbox num :', basket.EAN.index(self.search_entry.get()), ' to ', basket.number_of_items[basket.EAN.index(self.search_entry.get())])
            ...
            try:
                ...
                for file in os.listdir(filepath):
                    if file.endswith(".csv"):
                        ...
                        for barcode in barcodelist:
                            ...
                            with open(filepath+'/'+file, 'r', newline='', encoding='UTF8') as f:
                                reader = csv.reader(f, delimiter=";")
                ...
                showinfo('Streckkodsläsare', "Inget internet, fungerade ej")
            self.search_entry.delete(0, 30)
...
class DatabaseAdding:
    ...
    def add_free_article(self, window):
        if len(self.barcode.get()) > 0:
            ...
            # Find the article in the database
            filepath = '//kontoret/delad_mapp/'
            filename = filepath + "tillagda_streckkkoder.csv"
            file_exists = os.path.isfile(filename)
            if file_exists:
                pass
            else:
                showinfo("Streckkodsläsare", "Inget internet eller nåt")
            ...
            with open(filename, 'a', newline='', encoding='UTF8') as f:
                ...
                fields = [self.artnr.get(), str(self.description_entry.get()), float(self.price_entry.get()) / 1.25, self.barcode.get()]
